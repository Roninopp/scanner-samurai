...
@ubot.on_message(filters.command("scan", prefixes="?") & ~filters.private)
async def ubscan(_: Update, message: Message):

    user_id = message.from_user.id

    if user_id not in SUPPORT_USERS:
        return await message.reply_text("Only Enforcer can use this.")
    ...
    if flag == "-R":
        await pbot.send_photo(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="Accept", callback_data="ubaccept_call"),
                        InlineKeyboardButton(text="Reject", callback_data="ubreject_call")
                    ],
                    [
                        InlineKeyboardButton(text="Event", url=f"https://t.me/{SUPPORT_CHAT}/{message.id}")
                    ]
...
@ubot.on_callback_query(filters.regex("ubaccept_call"))
async def ubabout_commands_callbacc(_, CallbackQuery):
...
@ubot.on_callback_query(filters.regex("ubreject_call"))
async def ubabout_commands_callbacc(_, CallbackQuery):
    sender_id = CallbackQuery.from_user.id
    if sender_id in SUDO_USERS:
        await CallbackQuery.message.edit_caption(
...
@ubot.on_message(filters.command("revert", prefixes="?") & ~filters.private)
async def ubrevert(_: Update, message: Message):
